...
export const I18N = {
  en: {
    ...
    claudeCodeDataNotice: 'Claude Code does not retain full historical data — older session logs are periodically cleaned up. Sessions shown here represent individual API requests, not conversation sessions. A single conversation may generate hundreds of API requests, so Cost / Session will appear lower than the actual per-conversation cost.',
  ...
  zh: {
    ...
    claudeCodeDataNotice: 'Claude Code 不会保留完整的历史数据——旧的会话日志会被定期清理。此处显示的会话数为单次 API 请求数，而非对话会话数。一次对话可能产生数百次 API 请求，因此单次费用会低于实际的单次对话费用。',
